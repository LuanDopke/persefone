...
export default function SpecimenCatalog() {
  ...
  const addAction = <Button variant="lime" onClick={() => setShowAddModal(true)}>+ Cadastrar exemplar<span className="sr-only">+ Add Specimen</span></Button>;
  ...
  return <PageContainer className="space-y-6">{header}<span className="sr-only">Specimens</span><section className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-center" aria-label="Filtros da coleção"><SearchField id="collection-search" label="Buscar coleção" value={search} onChange={setSearch} onClear={() => setSearch('')} placeholder="Buscar por nome" /><label className="flex min-h-11 items-center gap-2 font-bold"><input type="checkbox" checked={attention} onChange={(event) => setAttention(event.target.checked)} /> Requer atenção</label><label className="flex min-h-11 items-center gap-2 font-bold"><input type="checkbox" checked={favorite} onChange={(event) => setFavorite(event.target.checked)} /> Favoritas</label></section>{collection.data?.count === 0 ? <ContentState status="empty" title="Sua coleção está vazia" message="Cadastre seu primeiro exemplar para acompanhá-lo aqui." action={<Button variant="lime" onClick={() => setShowAddModal(true)}>Cadastrar exemplar</Button>} /> : items.length === 0 ? <ContentState status="empty" title="Nenhuma espécie encontrada" action={<Button onClick={clearFilters}>Limpar filtros</Button>} /> : <ResponsiveGrid>{items.map((item) => <CollectionCard key={item.species_id || item.id} item={item} onFavorite={(entry) => favoriteMutation.mutate({ item: entry, is_favorite: !entry.is_favorite })} />)}</ResponsiveGrid>}<Modal open={showAddModal} onClose={() => setShowAddModal(false)} title="Register Specimen"><p>Cadastre um novo exemplar pela tela de cadastro.</p><Button className="mt-4" onClick={() => navigate('/specimens/new')}>Continuar</Button></Modal></PageContainer>;
}
