...
export default function IdentificationKeysPage() {
  ...
  return <PageContainer width="wide" className="space-y-7">
    <PageHeader eyebrow="Identificação colaborativa" title="Chaves de identificação" description="Uma chave orienta a identificação por características observáveis, como folhas, caule e flores. Escolha uma chave do grupo da planta e responda aos passos; o resultado será guardado como um palpite na observação." primaryAction={<div className="flex flex-wrap gap-2"><Link to="/keys/new" className="inline-flex border-4 border-charcoal bg-lime px-5 py-2.5 font-bold uppercase shadow-hard focus-visible:ring-4 focus-visible:ring-lime">Criar chave</Link><Button variant="secondary" onClick={() => setImportTarget({ source_kind: 'sdd' })}>Importar SDD</Button></div>} />
    ...
    {!mine && discoveryTerm.length >= 2 && <section className="space-y-4" aria-label="Chaves encontradas em fontes externas"><div className="border-b-2 border-charcoal/25 pb-3"><h2 className="text-xl font-bold uppercase">Fontes externas</h2><p className="mt-1 text-sm text-charcoal/70">Resultados da KeyBase e da Plazi. A chave só entra no catálogo local depois de importada, revisada e publicada.</p></div>
