...
export default function ObservationDetailPage() {
  ...
  return <PageContainer width="wide" className="space-y-8">
    ...
    <PageHeader eyebrow={data.species_detail ? 'Espécie confirmada' : 'Identificação em andamento'} title={data.title} description={data.species_detail?.scientific_name || `Observada em ${formatObservationDate(data.observed_at)}`} secondaryActions={<Button size="sm" variant="secondary" onClick={startEdit}>Editar ficha</Button>} />
    ...
    <section className="grid gap-5 border-b-2 border-charcoal/25 pb-7 md:grid-cols-[minmax(15rem,0.8fr)_minmax(0,1.2fr)]" aria-label="Resumo da observação">
      ...
      <div className="flex flex-col justify-center gap-4">
        ...
        <div className="flex flex-wrap gap-2"><Button size="sm" variant="lime" onClick={() => openComposer('hypotheses')}>Novo palpite</Button><Button size="sm" variant="secondary" onClick={() => openComposer('evidence')}>Adicionar evidência</Button></div>
      </div>
    </section>
