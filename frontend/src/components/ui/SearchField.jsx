...
export default function SearchField({ label, value, onChange, onClear, id = 'search-field', placeholder = 'Buscar', className = '' }) {
  return (
    <div className={`relative ${className}`}>
      ...
      {value && <button type="button" onClick={onClear} className="absolute right-2 top-1/2 min-h-11 -translate-y-1/2 border-2 border-charcoal px-3 font-bold focus-visible:ring-4 focus-visible:ring-lime" aria-label={`Limpar ${label.toLocaleLowerCase()}`}>Limpar</button>}
    </div>
  );
}
