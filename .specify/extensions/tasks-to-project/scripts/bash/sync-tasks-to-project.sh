#!/usr/bin/env bash
# sync-tasks-to-project.sh
#
# Wrapper around the Python implementation that syncs a Spec Kit
# tasks.md against an existing GitHub Project (v2) kanban board.
#
# Usage:
#   sync-tasks-to-project.sh [tasks_md_path] [--dry-run]
...
if [[ -z "$_python" ]]; then
  echo "tasks-to-project: Python 3 not found on PATH; cannot sync." >&2
  exit 1
fi

exec "$_python" "$PY_SCRIPT" sync "$@"
